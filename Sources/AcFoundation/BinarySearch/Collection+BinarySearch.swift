...
extension Range {
  ...
  @inlinable
  func left<Item>(_ x: Item, _ item: (Element) -> Item) -> Element
  where Item: Comparable, Bound: BinaryInteger {
  ...
  @inlinable
  func right<Item>(_ x: Item, _ item: (Element) -> Item) -> Element
  where Item: Comparable, Bound: BinaryInteger {
    var (left, right) = (startIndex, endIndex)
    while left < right {
      let mid = (left + right) >> 1
...
extension Collection where Index == Int {
  ...
  @inlinable
  func right<T>(_ x: T, start left: Index, end right: Index, _ key: (Element) -> T) -> Index
  where T: Comparable {
    var (left, right) = (startIndex, endIndex)
    while left < right {
      let mid = (left + right) >> 1
  ...
  @inlinable
  func left<T>(_ x: T, start left: Index, end right: Index, _ key: (Element) -> T) -> Index
  where T: Comparable {
    var (left, right) = (left, right)
    while left < right {
      let mid = (left + right) >> 1
